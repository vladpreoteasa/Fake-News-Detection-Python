...
vectorizer = TfidfVectorizer(max_features=30, stop_words=["english", "french", "spanish", "german", "italian"])
...
model = KNeighborsClassifier(n_neighbors=3000)
...
#     max_prob = np.max(test_probs[i])  # The proportion of neighbors that agree with the prediction
#     if max_prob < bias_parameter and test_preds[i] == 1:
#         test_preds[i] = 0  # Changing to negative for low confidence positive
